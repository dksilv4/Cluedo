...
public class ClueGUI extends Application {
    ...
    private HashMap<PlayerPiece, VBox> generateCharactersDetectiveSlipPanels(
            List<PlayerPiece> playerPieces, Stage theStage) {
        HashMap<PlayerPiece, VBox> dSlipPanels = new HashMap<>();
        for (PlayerPiece pp : playerPieces) {
            dSlipPanels.put(pp, generateDetectiveSlipPanel(pp, theStage));
        }
        return dSlipPanels;
    }
     ...
     */
    private VBox generateDetectiveSlipPanel(PlayerPiece pp, Stage theStage) {
    ...
    private HBox initActionsPane(Cluedo model) {
        ...
        rollDice.setOnMouseClicked(event -> {
            model.rollDiceB(model.state(), model.getPlayOrder());
        });
